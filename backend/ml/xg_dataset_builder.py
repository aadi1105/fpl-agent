...
logger = logging.getLogger("xg_dataset_builder")
...
class HistoricalXGDatasetBuilder:
    def __init__(self, raw_dir: str = RAW_DATA_DIR, output_dir: str = OUTPUT_DIR):
        self.raw_dir = raw_dir
        self.output_dir = output_dir
        os.makedirs(self.output_dir, exist_ok=True)
        # Load production expected minutes predictor for pre-deadline xMins feature
    ...
    def process_season(self, season: str, df: pd.DataFrame) -> pd.DataFrame:
        """Process a single season into per-fixture leak-free xG features and targets."""
        logger.info(f"Processing season {season} ({len(df)} raw rows) for xG dataset...")
        ...
        # Attacking features shift (goals, expected_goals, threat, creativity)
        ...
        if 'expected_goals' in fix_df.columns:
            ...
            fix_df['xg_shift'] = fix_df['goals_shift'] * 0.85
        ...
        if 'threat' in fix_df.columns:
            ...
            fix_df['threat_shift'] = fix_df['xg_shift'] * 100.0
        ...
        if 'creativity' in fix_df.columns:
            ...
            fix_df['creativity_shift'] = 0.0

        if 'penalties_missed' in fix_df.columns:
            fix_df['pen_miss_shift'] = fix_df.groupby('element')['penalties_missed'].shift(1).fillna(0)
        else:
            fix_df['pen_miss_shift'] = 0
        ...
        # Rolling Goals
        fix_df['goals_last_1'] = fix_df['goals_shift'].astype(float)
        fix_df['goals_last_3'] = fix_df.groupby('element')['goals_shift'].transform(lambda s: s.rolling(3, min_periods=1).sum()).astype(float)
        fix_df['goals_last_5'] = fix_df.groupby('element')['goals_shift'].transform(lambda s: s.rolling(5, min_periods=1).sum()).astype(float)
        fix_df['goals_last_10'] = fix_df.groupby('element')['goals_shift'].transform(lambda s: s.rolling(10, min_periods=1).sum()).astype(float)

        # Rolling xG
        fix_df['xg_last_1'] = fix_df['xg_shift'].astype(float).round(3)
        fix_df['xg_last_3'] = fix_df.groupby('element')['xg_shift'].transform(lambda s: s.rolling(3, min_periods=1).sum()).astype(float).round(3)
        fix_df['xg_last_5'] = fix_df.groupby('element')['xg_shift'].transform(lambda s: s.rolling(5, min_periods=1).sum()).astype(float).round(3)
        fix_df['xg_last_10'] = fix_df.groupby('element')['xg_shift'].transform(lambda s: s.rolling(10, min_periods=1).sum()).astype(float).round(3)

        # Rolling Threat & Creativity
        ...
        fix_df['threat_last_10'] = fix_df.groupby('element')['threat_shift'].transform(lambda s: s.rolling(10, min_periods=1).sum()).astype(float).round(1)
        ...
        # Per 90 attacking rates over last 5 matches
        mins_5 = fix_df['minutes_last_5'].replace(0, np.nan)
        fix_df['goals_per_90_last_5'] = ((fix_df['goals_last_5'] / mins_5) * 90.0).fillna(0.0).round(3)
        fix_df['xg_per_90_last_5'] = ((fix_df['xg_last_5'] / mins_5) * 90.0).fillna(0.0).round(3)
        fix_df['threat_per_90_last_5'] = ((fix_df['threat_last_5'] / mins_5) * 90.0).fillna(0.0).round(1)
        ...
        for c in rolling_cols:
            fix_df.loc[gw1_mask, c] = 0.0 if 'per_90' in c or 'xg' in c or 'threat' in c or 'creativity' in c else 0

        # Prepare features for vectorized expected minutes prediction
        ...
        # Add opponent names and team ratings
        ...
        # Batch predict expected minutes & availability
        ...
        fix_df['target_goals'] = fix_df['goals_scored'].astype(int)
        fix_df['actual_xg'] = fix_df.get('expected_goals', fix_df['goals_scored'] * 0.85).astype(float).round(3)
        ...
        out_cols = [
            ...
            "threat_last_5", "threat_last_10", "creativity_last_5",
            "goals_per_90_last_5", "xg_per_90_last_5", "threat_per_90_last_5",
            "target_goals", "actual_xg", "split"
        ]

        res_df = fix_df[out_cols].copy()
        logger.info(f"Completed season {season}: {len(res_df)} fixture rows created.")
        return res_df
        logger.info(f"Completed season {season}: {len(res_df)} fixture rows created.")
        return res_df

    def build_dataset(self) -> str:
        """Build complete historical xG dataset across all seasons."""
        all_dfs = []
        ...
        out_csv = os.path.join(self.output_dir, "historical_xg_dataset.csv")
        full_df.to_csv(out_csv, index=False)
        logger.info(f"Saved historical xG dataset to {out_csv} ({len(full_df)} total fixture rows).")
        ...
        meta = {
            ...
            "leak_audit": "VERIFIED (All rolling stats shifted by 1, GW1 reset to 0, per-fixture targets)"
        }

        meta_path = os.path.join(self.output_dir, "xg_dataset_metadata.json")
