...
def test_authoritative_current_gw_truth(db_session):
...
def test_expected_minutes_no_clustering_75(db_session):
    """Verify expected minutes does not suffer from artificial 75.0 clustering."""
    players = db_session.query(Player).filter(Player.element_type.in_(["MID", "FWD"])).limit(50).all()
    auditor = DecisionEngineRealityAuditor(db_session)
    xmins_vals = [auditor.proj_engine.calculate_expected_minutes(p) for p in players]
    
    # Verify variety of expected minutes values (not all 75.0)
    unique_vals = set(xmins_vals)
    assert len(unique_vals) >= 3, f"Expected minutes lacks diversity: {unique_vals}"

def test_gyokeres_vs_havertz_selection_mechanics(db_session):
    """Verify first-principles selection mechanics evaluation of Gyokeres vs Havertz."""
    auditor = DecisionEngineRealityAuditor(db_session)
    audit = auditor.audit_gyokeres_vs_havertz()
    assert audit["gyokeres"]["found"] == True
    assert audit["havertz"]["found"] == True
    assert "preference_reason" in audit

def test_long_term_injury_exclusion(db_session):
    """Verify long-term injured or unavailable players are not optimizer eligible."""
    mock_inj = Player(
        id=999992,
        web_name="TestInjured2",
        element_type="FWD",
        now_cost=85,
        status="i",
...
def test_my_team_crud_and_persistence(db_session):
...
def test_selection_trace_api(db_session):
...
def test_fixture_sensitivity_impact(db_session):
    """Verify controlled diagnostic comparing actual vs raw baseline fixture impact."""
    auditor = DecisionEngineRealityAuditor(db_session)
    sens = auditor.audit_fixture_sensitivity(["Haaland", "Saka", "Palmer"])
    assert len(sens) >= 3
    for item in sens:
